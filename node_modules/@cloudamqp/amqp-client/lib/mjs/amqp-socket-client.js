    ...
    connect() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const onConnect = (conn) => {
                socket.setTimeout(this.heartbeat * 1000);
                resolve(conn);
            };
        ...
        while (bufPos < bufLen) {
            if (this.frameSize === 0) {
                if (this.framePos !== 0) {
                    ...
                    if (copied === 0)
                        throw new AMQPError(`Copied 0 bytes framePos=${this.framePos} bufPos=${bufPos} bytesWritten=${bufLen}`, this);
            ...
            if (copied === 0)
                throw new AMQPError(`Copied 0 bytes, please report this bug, frameSize=${this.frameSize} framePos=${this.framePos} bufPos=${bufPos} copyBytes=${copyBytes} bytesWritten=${bufLen}`, this);
            this.framePos += copied;
            bufPos += copied;
